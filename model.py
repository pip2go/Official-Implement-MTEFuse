...
class MLP(nn.Module):
    def __init__(self,
        ...
        self.dwsconv = DWSConv(hidden_features * 2, hidden_features * 2, kernel_size=3, stride=1, padding=1)
    ...
    def forward(self, x):
        x = self.module_in(x)
        x1, x2 = self.dwsconv(x).chunk(2, dim=1)
        x = F.silu(x1) * x2
        x = self.norm(x)
        x = self.module_out(x)
        return x
...
class TRCNB(nn.Module):
    def __init__(self, in_dim, out_dim):
        ...
        self.GFE = GlobalFeatureExtractor(dim=out_dim, in_dim=in_dim, num_heads=8)
        self.LFE = LocalFeatureExtractor(dim=out_dim)
...
class GlobalFeatureExtractor(nn.Module):
    def __init__(self, dim, in_dim, num_heads, qkv_bias=False):
        super(GlobalFeatureExtractor, self).__init__()
        self.norm1 = LayerNorm(dim)
        self.attn = Transformer_Attention(dim, in_dim, num_heads=num_heads, qkv_bias=qkv_bias)
...
########################################################################################################################################
class MUPAtt(nn.Module):
    def __init__(self, dim, G=2):
        super().__init__()
        self.G = G
        self.channel = dim
        # self.gn = nn.GroupNorm(dim // (4 * G), dim // (2 * G))
        self.gn = nn.GroupNorm(num_groups=2, num_channels=2)
        # 可学习参数
        self.cweight = Parameter(torch.zeros(1, dim // (2 * G), 1, 1))
        self.cbias = Parameter(torch.ones(1, dim // (2 * G), 1, 1))
        self.sweight = Parameter(torch.zeros(1, dim // (2 * G), 1, 1))
        self.sbias = Parameter(torch.ones(1, dim // (2 * G), 1, 1))
        self.sigmoid = nn.Sigmoid()
        # 只保留一个投影层
    ...
    def forward(self, x):
        b, c, h, w = x.size()
        # 分组
        indentity = self.proj_out(x)
        x = x.view(b * self.G, -1, h, w)  # [b*G, c//G, h, w]
        x10, x11 = x.chunk(2, dim=1)  # 每部分 [b*G, c//(2*G), h, w]

        # 分组归一化，无需.to(device)
        x10_spatial = self.gn(x10)
        x11_spatial = self.gn(x11)

        # 应用可学习的缩放和偏置
        x10_spatial = self.sweight * x10_spatial + self.sbias
        x11_spatial = self.sweight * x11_spatial + self.sbias

        # 门控
        x10_spatial_Sig = x10 * self.sigmoid(x11_spatial)
        x11_spatial_Sig = x11 * self.sigmoid(x10_spatial)

        # 拼接并恢复形状
        out = torch.cat([x10_spatial_Sig, x11_spatial_Sig], dim=1)  # [b*G, c//G, h, w]
        out = out.contiguous().view(b, -1, h, w)  # [b, c, h, w]

        # 投影并洗牌
        out = self.proj_out(out)+indentity # [b, c//2, h, w]
        return out
...
class Transformer_Attention(nn.Module):
    def __init__(self,
        ...
        # MixUPAtt,混合注意力
        self.MUPAtt1 = MUPAtt(8)
        self.proj_q = nn.Conv2d(dim, dim, kernel_size=3, stride=1, padding=1)

        self.Q1_Conv = nn.Conv2d(dim, dim, kernel_size=3, stride=1, padding=1)
        self.K1_Conv = nn.Conv2d(dim, dim, kernel_size=3, stride=1, padding=1)
        self.V1_Conv = nn.Conv2d(dim, dim, kernel_size=3, stride=1, padding=1)

        self.Q2_Conv = nn.Conv2d(dim, dim, kernel_size=3, stride=1, padding=1)
        self.K2_Conv = nn.Conv2d(dim, dim, kernel_size=3, stride=1, padding=1)
        self.V2_Conv = nn.Conv2d(dim, dim, kernel_size=3, stride=1, padding=1)
    ...
    def _init_weights(self):
        # 初始化卷积层
        for conv in [self.Q1_Conv, self.K1_Conv, self.V1_Conv,
                     self.Q2_Conv, self.K2_Conv, self.V2_Conv,self.dw_conv]:
            if isinstance(conv, nn.Conv2d):  # 确保是卷积层
    ...
    def _reshape(self, x: torch.Tensor, H: int, W: int, cha_exp=None) -> torch.Tensor:
        """将特征图转换为多头注意力需要的形状"""
        B, C = x.shape[:2]
        if not cha_exp:
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        # 独立的 Q/K/V 投影
        q1 = self.Q1_Conv(x1)  # [B, C, H, W]
        k1 = self.K1_Conv(x1)
        v1 = self.V1_Conv(x1)

        # 转换为多头表示
        q1 = torch.nn.functional.normalize(self._reshape(q1, H, W), dim=-1)  # [B, num_heads, head_dim, (H*W)]
        k1 = torch.nn.functional.normalize(self._reshape(k1, H, W, self.cha_exp), dim=-1)
        v1 = torch.nn.functional.normalize(self._reshape(v1, H, W, self.cha_exp), dim=-1)

        # 独立的 Q/K/V 投影
        q2 = self.Q2_Conv(x2)  # [B, C, H, W]
        k2 = self.K2_Conv(x2)
        v2 = self.V2_Conv(x2)

        # 转换为多头表示
        q2 = torch.nn.functional.normalize(self._reshape(q2, H, W), dim=-1)  # [B, num_heads, head_dim, (H*W)]
        k2 = torch.nn.functional.normalize(self._reshape(k2, H, W), dim=-1)
        v2 = torch.nn.functional.normalize(self._reshape(v2, H, W), dim=-1)

        q = q1+q2
        q = self.MUPAtt1(q)

        # qkv1运算
        attn1 = (q @ k1.transpose(-2, -1)) * self.scale # [B, num_heads, head_dim, head_dim]
        attn1 = attn1.softmax(dim=-1)
        attn1 = self.attn_drop(attn1)
        out1 = (attn1 @ v1) # [B, num_heads, head_dim, (H*W)]
        # 合并多头
        out1 = out1.contiguous().view(B, C, H, W) # [B, (num_heads*head_dim), H, W]

        # qkv2运算
        attn2 = (q @ k2.transpose(-2, -1)) * self.scale # [B, num_heads, head_dim, head_dim]
        attn2 = attn2.softmax(dim=-1)
        attn2 = self.attn_drop(attn2)
        out2 = (attn2 @ v2) # [B, num_heads, head_dim, (H*W)]
        # 合并多头
        out2 = out2.contiguous().view(B, C, H, W) # [B, (num_heads*head_dim), H, W]

        out = self.proj_q(out1 + out2)
...
class Spatial_Attention(nn.Module):
    def __init__(self, channels, factor=32):
        ...
        # self.innorm = nn.InstanceNorm2d(channels // self.groups, affine=True)
    ...
    def forward(self, x):
        ...
        x_h, x_w = torch.split(hw, [h, w], dim=2)       # b*G, c//G, 1, h       # b*G, c//G, 1, w
        ...
        x2_h = self.pool_h(x2)  # b*G, c//G, 1, w
        x2_w = self.pool_w(x2).permute(0, 1, 3, 2)  # b*G, c//G, 1, h
        x2_hw = self.conv1x1(torch.cat([x2_h, x2_w], dim=2))  # b*G, c//G, 1, w+h
        x2_h, x2_w = torch.split(x2_hw, [h, w], dim=2)  # b*G, c//G, 1, h       # b*G, c//G, 1, w
        x2_hw_end = self.gn(group_x * x2_h.sigmoid() * x2_w.permute(0, 1, 3, 2).sigmoid())  # b*G, c//G, h, w
        x12 = x2_hw_end.reshape(b * self.groups, c // self.groups, -1)       # b*G, c//G, h*w
        ...
        enhanced = group_x * weights.sigmoid()  # b*G, c//G, h, w
        # return (0.8 * enhanced + 0.2 * group_x).reshape(b, c, h, w)  # b, c, h, w
        # 添加细节保留分支
        detail_preserved = self.detail_preserve(x)
        # 结合增强和细节保留
        result = (0.6 * enhanced + 0.4 * detail_preserved.reshape(b * self.groups, c // self.groups, h, w))
        return result.reshape(b, c, h, w)

class CHSP(nn.Module):
    def __init__(self, channels, factor=32, reduction=4):
        super().__init__()
        self.spatial = Spatial_Attention(channels, factor=factor)
        self.sigmoid = nn.Sigmoid()
    ...
    def forward(self, x):
        channel = self.sigmoid(self.channel_MAX(x) + self.channel_AVG(x)) * x
        spatial_weight = self.spatial(x)
        spatial = x * spatial_weight
...
class DecoderConvlution(nn.Module):
    def __init__(self, in_c, out_c, k, s, pad=True, activation='prelu'):
        ...
        self.conv = nn.Sequential(nn.Conv2d(in_c, out_c, k, s),
...
class MTEFuse_model(nn.Module):
    ...
    def forward(self, x1, x2):
        x1 = self.encoder(x1)
        x2 = self.encoder(x2)
        x11 = self.fuses1(torch.cat((x1[0], x2[0]), dim=1))
        x22 = self.fuses2(torch.cat((x1[1], x2[1]), dim=1))
        x33 = self.fuses3(torch.cat((x1[2], x2[2]), dim=1))
        x44 = self.fuses4(torch.cat((x1[3], x2[3]), dim=1))
