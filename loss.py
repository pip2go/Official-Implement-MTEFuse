...
def gradient_loss(pred, *target):
    """
        梯度损失
    """
    Grad_total = torch.zeros_like(SpatialGradient()(pred))
    l1Loss = nn.L1Loss()
    if len(target)>1:
        for item in target:
            Grad_total+=SpatialGradient()(item)
        Grad_total /= len(target)
        return l1Loss(SpatialGradient()(pred), Grad_total)
    else:
        Gradient_loss = l1Loss(SpatialGradient()(pred), SpatialGradient()(target[0]))
        return Gradient_loss

def gradient_loss_max(pred, *target):
    """
    使用最大梯度图进行对齐，保留细节
    """
    gradients = [SpatialGradient()(item) for item in target]
    grad_target = torch.max(torch.stack(gradients), dim=0)[0]  # 取最大
    return F.l1_loss(SpatialGradient()(pred), grad_target)
